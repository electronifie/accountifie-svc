...
Account.prototype.cpBalances = function (options) {
  ...
  var opening = this._balanceDelta({ cps: true, to: parseDate(from).clone().subtract(1, 'day').format('YYYY-MM-DD'), skipCache: skipCache, filter: filter });
  var closing = this._balanceDelta({ cps: true, to: to, skipCache: skipCache, filter: filter });
...
Account.prototype._balanceDelta= function (options) {
  ...
  log.info('RUNNNING', options);
  if (byCP) {
    var lines = [].concat.apply([], transactions.map(function(tr) {
                                                      return tr.lines}));
    var gpd = _.groupBy(lines, "counterpartyId");
    var cpBals = Object.keys(gpd).map(function(key, index) {
                                     return {'cp': key,
                                            'total': gpd[key].reduce(function(memo, l) { 
                                                                      return memo.plus(l.amount)
                                                                      }, 
                                                                      new BigNumber(0))}
    });
    log.info('CPBALS YEAH');
    log.info(cpBals);
    return cpBals;
  }
