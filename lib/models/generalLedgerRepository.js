    ...
    .toArray(function (err, snapshots) {
      ...
      self.events.find(criteria)
        ...
        .toArray(function (err, events) {
          ...
          log.trace('Deserializing %s events %s', events.length, (snapshot ? 'over snapshot at v' + snapshot.version : 'without snapshot'));
          var entity = self._deserialize(id, snapshot, events);
          entity.timestamp = (_.last(events) || { timestamp: Date.now() }).timestamp
...
GeneralLedgerRepository.prototype.hasSnapshotFor = function (id, date, cb) {
  var self = this;
  this.snapshots
    .find({ id: id, timestamp: { $lte: date.getTime() } })
    .sort({ version: -1 })
    ...
    .toArray(function (err, snapshots) {
      ...
      var snapshot = snapshots[0];
      if (!snapshot) return cb(null, false);

      self.events.count({
        id: id,
        version: { $gt: snapshot.version },
        timestamp: { $lte: date.getTime() }
      }, function (err, count) {
        if (err) return cb(err);
        cb(null, count === 0);
      });
